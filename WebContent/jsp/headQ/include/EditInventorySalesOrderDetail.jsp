		                ...
		                <s:iterator value="formBean.order.product_List" status = "st" id="orderProduct" >
						 	<tr id="row<s:property value="#st.index"/>"  class="excelTable">
						 		...
						 		<td><select id="priceSlect<s:property value="#st.index"/>" name="formBean.order.product_List[<s:property value="#st.index"/>].salePriceSelected" onchange="onWholeSalePriceChange(3,<s:property value="#st.index"/>);">
	                                   ...
	                                   <option value='<s:property value="#orderProduct.productBarcode.product.salesPriceFactory"/>' <s:if test="#orderProduct.productBarcode.product.salesPriceFactory == #orderProduct.salePriceSelected">selected</s:if>>零售价 <s:text name="format.price"><s:param value="#orderProduct.productBarcode.product.salesPriceFactory"/></s:text></option>		        
							        </select></td>
